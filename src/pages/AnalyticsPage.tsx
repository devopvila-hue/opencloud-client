...
export default function AnalyticsPage() {
  ...
  const tasksByDept = useMemo(() => {
    ...
    for (const task of tasks.data ?? []) map.set(task.department_key, (map.get(task.department_key) ?? 0) + 1);
    ...
    <div className="mx-auto w-full max-w-7xl space-y-6 p-4 md:p-6 lg:p-8">
      ...
      <Card>
        ...
        <dl className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <Field label={t('analytics.system.gateway_url')} value={system.data?.gateway.gateway_url ?? '—'} />
          <Field label={t('analytics.system.gateway_latency')} value={formatDuration(system.data?.gateway.latency_ms ?? null)} />
          <Field label={t('analytics.system.supabase_latency')} value={formatDuration(system.data?.supabase.latency_ms ?? null)} />
          <Field label={t('analytics.system.models_available')} value={String(system.data?.gateway.models.length ?? 0)} />
          <Field label={t('analytics.system.active_model')} value={system.data?.gateway.agent_model ?? '—'} />
          <Field label={t('analytics.system.middleware_version')} value={system.data?.middleware_version ?? '—'} />
        </dl>
      </Card>
    </div>
  );
